...
def _agora() -> str: return datetime.now(timezone.utc).isoformat()
def criar_lote_instagram(supabase: Client, produto_ids: list[int], source_chat_id: str | None = None, source_message_id: int | None = None) -> int | None:
    if not INSTAGRAM_AUTO_BATCH or not produto_ids: return None
    ...
    for inicio in range(0, len(ids), INSTAGRAM_BATCH_SIZE):
        ...
        if primeiro_post_id is None: primeiro_post_id = post_id
...
def registrar_produto_processado(supabase: Client, produto_id: int, produto: dict[str, Any]) -> list[int]:
    ...
    post_ids=[]; snapshot=dict(produto); snapshot.pop("raw_response",None)
...
def _buscar_produtos_do_lote(supabase: Client, post_id: int) -> list[dict[str, Any]]:
    ...
    for row in rows.data or []:
        if isinstance(row.get("product_snapshot"),dict): produto=dict(row["product_snapshot"]); produto["id"]=row.get("produto_fila_id"); produtos.append(produto)
    return produtos
def _extrair_attachments(detail: dict[str, Any], messages: dict[str, Any] | None = None) -> list[dict[str, str]]:
    ...
    def adicionar(item):
        ...
        nome=str(item.get("file_name") or item.get("filename") or item.get("name") or "imagem").strip(); chave=(nome,url)
        if chave not in vistos: vistos.add(chave); encontrados.append({"file_name":nome,"url":url})
    def percorrer(value):
        if isinstance(value,dict):
            adicionar(value)
            for v in value.values(): percorrer(v)
        elif isinstance(value,list):
            for v in value: percorrer(v)
    percorrer(detail)
    if messages: percorrer(messages)
    return encontrados
def _salvar_file_ids(supabase: Client | None, post_id: int, file_ids: list[str]) -> None:
    if supabase is None or not file_ids: return
    try:
        row=supabase.table("instagram_posts").select("assets").eq("id",post_id).limit(1).execute(); assets=row.data[0].get("assets") if row.data else []
        if not isinstance(assets,list): return
        novos=[]
        for idx,asset in enumerate(assets):
            item=dict(asset) if isinstance(asset,dict) else {"asset_url":str(asset)}
            if idx<len(file_ids): item["telegram_file_id"]=file_ids[idx]
            novos.append(item)
        supabase.table("instagram_posts").update({"assets":novos,"updated_at":_agora()}).eq("id",post_id).execute()
    except Exception: logger.exception("Não foi possível salvar file_id do Telegram para o lote #%s.",post_id)
def _enviar_preview_telegram(post_id:int,detail:dict[str,Any],attachments:list[dict[str,str]],caption:str,task_url:str|None,supabase:Client|None=None)->bool:
    ...
    base=f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"; enviados=0; file_ids=[]
    for inicio in range(0,len(attachments),10):
        grupo=attachments[inicio:inicio+10]; media=[]
        for pos,item in enumerate(grupo):
            parte={"type":"photo","media":item["url"]}
            if inicio==0 and pos==0: parte["caption"]=f"🦊 <b>CARROSSEL #{post_id}</b>\n\n{caption[:900]}"; parte["parse_mode"]="HTML"
            media.append(parte)
        resposta=requests.post(f"{base}/sendMediaGroup",json={"chat_id":int(TELEGRAM_ADMIN_ID),"media":media},timeout=60)
        if not resposta.ok: logger.error("Telegram recusou preview do lote #%s: %s",post_id,resposta.text[:1000]); return False
        try:
            for msg in resposta.json().get("result",[]):
                photos=msg.get("photo") or []
                if photos: file_ids.append(str(photos[-1].get("file_id") or ""))
        except Exception: logger.exception("Não foi possível extrair file_ids do Telegram para o lote #%s.",post_id)
        enviados+=len(grupo)
    ...
    if not resposta.ok: logger.error("Telegram recusou os botões do preview do lote #%s: %s",post_id,resposta.text[:1000])
    return resposta.ok
def processar_lote_se_pronto(supabase:Client,post_id:int)->bool:
    ...
    if len(produtos)<INSTAGRAM_BATCH_SIZE: return False
    ...
    try:
        ...
        supabase.table("instagram_posts").update({"manus_task_id":task_id,"manus_task_url":task_url,"prompt":"Carrossel Instagram criado automaticamente pela pipeline; preview enviado ao Telegram quando concluído.","updated_at":_agora()}).eq("id",post_id).execute(); logger.info("Lote Instagram #%s enviado para Manus: %s",post_id,task_id); return True
...
def processar_webhook_manus(supabase:Client,payload:dict[str,Any])->tuple[bool,str]:
    ...
    if event_type=="task_stopped":
        if detail.get("stop_reason")=="finish":
            try:
                ...
                value=structured.get("value") or {}; attachments=_extrair_attachments(detail)
                if not attachments:
                    try: attachments=_extrair_attachments({},listar_mensagens_tarefa(task_id))
                ...
                enviados=_enviar_preview_telegram(post_id,detail,attachments,str(value.get("caption") or ""),str(detail.get("task_url") or "") or None,supabase)
                return (True,"lote pronto e preview enviado ao Telegram" if enviados else "lote pronto; preview Telegram não enviado")
            except Exception as exc:
                supabase.table("instagram_posts").update({"status":"error","error":str(exc)[:4000],"updated_at":_agora()}).eq("id",post_id).execute(); return False,"falha ao recuperar resultado Manus"
        supabase.table("instagram_posts").update({"status":"error","error":detail.get("message") or "Manus interrompeu a tarefa.","updated_at":_agora()}).eq("id",post_id).execute(); return False,"tarefa Manus não concluída"
